...
def output_layer(input_layer, num_labels):
    ...
    fc_w = create_variables(name='fc_weights', shape=[input_dim, num_labels], initializer=tf.uniform_unit_scaling_initializer(factor=1.0), is_fc_layer=True)
...
def residual_block(input_layer, output_channel, first_block=False):
    ...
    if input_channel * 2 == output_channel:
        ...
        raise ValueError('Output channel and input channel does not match in residual blocks!!!')
...
def inference(input_tensor_batch, n, reuse):
    ...
    :param reuse: To build train graph, reuse = False. To build validation graph, and share weights with train graph, reuse = True.
